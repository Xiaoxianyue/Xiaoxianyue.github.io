import{_ as e}from"./plugin-vue_export-helper-DlAUqK2U.js";import{o as i,c as n,b as a}from"./app-Dj9lEr-e.js";const s="/assets/image-20240502171846391-qoI_sENA.png",t="/assets/image-20240502172911698-B2ZmrF7F.png",l="/assets/image-20240502173530113-B2-gSwNa.png",r="/assets/image-20240502173725062-D15xyW_y.png",c="/assets/image-20240502174004629-DDcCJiGX.png",d="/assets/image-20240502174502986-DOIFtl_3.png",o="/assets/image-20240502174900544-CHFXCZEl.png",m="/assets/image-20240502174952352-BbQJBXky.png",u="/assets/image-20240502193228617-ReaVif-l.png",v="/assets/image-20240506112014686-BTW7eSF7.png",p={},h=a('<h2 id="_1-public-methods" tabindex="-1"><a class="header-anchor" href="#_1-public-methods"><span>1. public Methods</span></a></h2><p>• This means that code running outside the class can make calls to that method</p><p>• This has got to be the case, since we want people to interact with our objects by calling methods in them.</p><p>• In general the rules are:</p><p>–if it is a data member (i.e. it holds data) of the class, make it private</p><p>–if it is a method member (i.e. it does something) make it public</p><ul><li><p>这意味着在类外运行的代码可以调用该方法</p></li><li><p>情况必须如此，因为我们希望人们通过调用对象中的方法来与我们的对象交互。</p></li><li><p>一般来说，规则如下</p></li></ul><p>-如果它是类的数据成员（即保存数据），则将其私有化</p><p>-如果是方法成员（即执行某些操作），则将其设置为公共成员</p><img src="'+s+'" alt="image-20240502171846391"><h2 id="_2-static-items" tabindex="-1"><a class="header-anchor" href="#_2-static-items"><span>2. Static Items</span></a></h2><p>• All the members that we have created in our class Account have been part of an instance of the class</p><p>• One can create members which are held as part of the class, i.e. they exist outside of any particular instance</p><p>• The static keyword lets us create members which are not held in an instance, but in the class itself</p><ul><li><p>我们在类账户中创建的所有成员都是类实例的一部分</p></li><li><p>我们可以创建作为类的一部分的成员，即它们存在于任何特定实例之外。</p></li><li><p>通过 static 关键字，我们可以创建不存在于实例中，而是存在于类本身的成员</p></li></ul><h3 id="_2-1-static-vs-const" tabindex="-1"><a class="header-anchor" href="#_2-1-static-vs-const"><span>2.1 Static vs const</span></a></h3><p>• Note that we have made the data member of the class static, so that it is part of the class and not an instance of the class.</p><p>• One common programming mistake is to confuse static with const</p><p>–Marking a variable as const means ―the value cannot be changed</p><p>–Marking a variable with static means ―the variable is part of the class and is always present</p><ul><li><p>请注意，我们将该类的数据成员设置为静态，因此它是该类的一部分，而不是该类的实例。</p></li><li><p>一个常见的编程错误是混淆 static 和 const</p></li></ul><p>-将变量标记为 const 意味着 - 变量值不可更改</p><p>-将变量标记为 static 意味着 - 变量是类的一部分，始终存在</p><h3 id="_2-2-共享main函数" tabindex="-1"><a class="header-anchor" href="#_2-2-共享main函数"><span>2.2 共享Main函数</span></a></h3><ul><li>AccountTest class 有一个名为 Main() 的静态成员方法</li><li>如果我们创建 50 个 AccountTest 实例，它们都将共享相同的 Main() 方法</li><li>在 C# 中，关键字 static 将成员标记为类的一部分，而不是类实例的一部分。</li></ul><figure><img src="'+t+'" alt="image-20240502172911698" tabindex="0" loading="lazy"><figcaption>image-20240502172911698</figcaption></figure><p>•We don&#39;t have to make an instance of the AccountTest class to be able to use the Main() method</p><p>• Keyword static doesn’t mean “cannot be changed”</p><p>• Members of a class which have been made static can be used just like any other member of a class • Either a data member or a method can be made static</p><p>-我们不必创建 AccountTest 类的实例就能使用 Main() 方法</p><ul><li><p>关键字 static 并不意味着 &quot;不能更改&quot;。</p></li><li><p>静态化后的类成员可以像其他成员一样使用 - 数据成员或方法都可以被静态化</p></li></ul><h3 id="_2-3-using-a-static-data-member-of-a-class" tabindex="-1"><a class="header-anchor" href="#_2-3-using-a-static-data-member-of-a-class"><span>2.3 Using a static data member of a class</span></a></h3><p>下面这个关于银行利率的代码：</p><p>• The interest rate is held for all the accounts. • If the interest rate charged it must be charged for all accounts.</p><ul><li>所有账户都有利率。 - 如果收取利率，则必须对所有账户收取。</li></ul><figure><img src="'+l+'" alt="image-20240502173530113" tabindex="0" loading="lazy"><figcaption>image-20240502173530113</figcaption></figure><p>我们可以这么解决这个问题：</p><figure><img src="'+r+'" alt="image-20240502173725062" tabindex="0" loading="lazy"><figcaption>image-20240502173725062</figcaption></figure><p>现在，这个利率是类的成员而不是实例的成员。我们要通过调用类的方法调用它。</p><figure><img src="'+c+'" alt="image-20240502174004629" tabindex="0" loading="lazy"><figcaption>image-20240502174004629</figcaption></figure><h3 id="_2-4-using-a-static-method-in-a-class" tabindex="-1"><a class="header-anchor" href="#_2-4-using-a-static-method-in-a-class"><span>2.4 Using a static method in a class</span></a></h3><p>什么情况下用Static Method？？</p><p>Example：</p><p>我们可以根据某人的年龄和收入来决定是否允许其拥有银行账户。</p><figure><img src="'+d+'" alt="image-20240502174502986" tabindex="0" loading="lazy"><figcaption>image-20240502174502986</figcaption></figure><p>这时我们要有了一个实例（账户）后才能调用该方法。但是我们不想这样。我们想调用这个实例并被允许后，才能激活满足条件的实例（账户）。</p><blockquote><p>•We can&#39;t call the method until we have an Account instance. •We can solve this by making the method static</p></blockquote><figure><img src="'+o+'" alt="image-20240502174900544" tabindex="0" loading="lazy"><figcaption>image-20240502174900544</figcaption></figure><p>现在这个方法是类的一部分，不属于任何实例；</p><figure><img src="'+m+`" alt="image-20240502174952352" tabindex="0" loading="lazy"><figcaption>image-20240502174952352</figcaption></figure><h3 id="_2-5-using-member-data-in-static-methods" tabindex="-1"><a class="header-anchor" href="#_2-5-using-member-data-in-static-methods"><span>2.5 Using member data in static methods</span></a></h3><div class="language-c# line-numbers-mode" data-ext="c#" data-title="c#"><pre class="language-c#"><code>using System;
namespace Program
{
    class Account
    {
